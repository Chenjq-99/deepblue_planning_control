 ...
 * @Last Modified time: 2022-07-10 22:37:29
 */
/*****************************************************************
 * @file   map.h
 * @brief  An OpenDrive-based HD-Map Data Interface oriented to autonomous
 *vehicle route planning.
 * @details This interface dedicats to provide autonomous vehicles' navigation
 *module access to the necessary map data and also the basic route planning
 *function. The ...
 *
 * @author Xingjiang Yu
 * @version V0.8
...
// ------------------------------------ Types
// redefined----------------------------------------------
typedef double RoadS;   // the distance between the begin of the road to a
                        // position on road along the reference line
typedef double RouteS;  // the distance between the begin of the route to a
                        // position on route along the reference lines
 ...
 * @brief enumrate the types of roadmark of lane（if roadmark has double
 * lines,pick the closest (to the lane center) one）
 */
typedef enum {
  kUnknownMark = 0,  //
  kBrokenWhite = 1,
...
/**
     * @enum LaneTurnType
     * @brief enumrate the possible lane turning options of a lane, compositions
   can be refelected by bits
     *      未知 kUnkonwnTurnType	= -1,
            ...
            所有皆可						= 0xF
    */
typedef int32_t LaneTurnTypeOrigin;  // NOTE: Standard OpenDrives do not have
                                     // Turn Type attributes
...
struct MAP_API LaneInfo {
  ...
  RoadS length;  //
  // LaneTurnType lane_turn_type; // NOTE: Currently TurnType feature is not
  // included in our map, hence all the turn_type you
  //							 // read will be
  //"kUnkonwnTurnType"
  LaneInfo();
};
 ...
 *  @brief a light-weighted traffic object information structure used for quick
 * search
 */
 ...
 * @struct StopLine
 * @brief Road reference line based  coordinate of a certain position
 * @warning One Stopline usually related to one light, sometimes multiple
 * lights(if one light group has only one stopline reference)
 */
struct MAP_API StopLine {
  ObjectId id;
  RoadId road_id;                // Global lane id
  TrafficLightGroupId group_id;  // NOTE: 目前认为一个group一个Stopline
                                 // 而不是一个trafficlight 一个 stopline
...
struct MAP_API TrafficLight {
  ...
  std::vector<ObjectId>
      reference_list;  // 红绿灯相关的Object的id的list，比如杆子,停止线
...
struct MAP_API Route {
  std::vector<LaneId>
      lane_id_vec;  // routing 所得的路线的Vector版，方便c++端调用
...
struct MAP_API TracePoint {
  ...
  double right_border;
  TracePoint() = default;
  TracePoint(double xx, double yy, double zz, LaneId ldld, double ss, double ll,
             double hh, double cc, double cdcd, SingleLaneRoadMarkType lr,
             SingleLaneRoadMarkType rr, TracePointType tpt, double left_border_,
             double right_border_);
};

typedef std::vector<TracePoint> ReferenceLineInfo;
...
enum ErrorCode {
  ...
  kAnchorInvalid = 6,
  kAnchorRelocated =
      7,  // 锚点重定位，当用户所指位置无锚点而自动分配给nearest point时返回
  kAnchorsTooClose = 8,
  kRoadSInvalid = 9,    //
  kRouteSInvalid = 10,  //
...
/**
 *  @class Map
 *  @brief Surface class of the route planning oriented OpenDrive HDMap
 * Interface
 *  @details
 */
class MAP_API Map {
 public:  // TODO:
  ...
  /**
   * @brief transform an UTM-based XYZ coordinate to Road-Reference-Line-based
   * SLZ coordinate
  ...
  /**
               * @brief transform an Road-Reference-Line-based SLZ coordinate to
     UTM-based XYZ coordinate
               * @param slz  SLZ coordinate system
  ...
  // ----------------------------------------------
  /**
   * @brief for users to manually create an anchor by specifing the anchor's id
   * and position
   * @param id  proposed anchor's id
   * @param pos  proposed anchor's position in SLZ coordinate system
   * @return the created anchor object, return ANCHOR_EMPTY() if no anchor was
   * created
   */
   ...
   * @param pattern pattern of the desired anchor's string id, could be writtern
   * in regular expression
   * @param anchor_list the anchor list to be searched in
   * @param anchor the urged anchor
   * @return true:success, false: failed
   */
  static ErrorCode find_anchor(const char*& pattern,
                               const AnchorArray& anchor_list,
                               AnchorArray& found_anchors);
   ...
   */
  ErrorCode query_lane_speed_at(const LaneId& id, const RoadS& s,
   ...
   */
  LaneChangeType query_lane_change_type_at(const LaneId& id,
                                           const RoadS& s) const;

  /**
   * @brief query the left and right boundary of the specified lane
   * @param lane_id  the global id of the lane to be queried
   * @param left_boundary  the sample vertices' SLZ coordinate list of the left
   * boundary
   * @param right_boundary  the sample vertices' SLZ coordinate list of the
   * right boundary
   * @return ErrorCode
   */
  ErrorCode query_lane_boundaries(const LaneId& lane_id,
                                  double sampling_spacing,
                                  SLZArray& left_boundary,
                                  SLZArray& right_boundary) const;

  // ErrorCode query_road_mark_between(const LaneId& id_1, const LaneId& id_2, )
  // const;
  /**
   ...
   * @param centerline the center line in the specified interval on the current
   * lane
   * @return ErrorCode
   */
  ErrorCode calc_lane_center_line(const LaneId& id, const RoadS& s1,
                                  const RoadS& s2, double sampling_spacing,
                                  SLZArray& centerline) const;

  ErrorCode get_ROI_lane_line(const XYZ& point, std::vector<XYZ>& roi_points,
                              const double distance = 20.0) const;

  ErrorCode get_ROI_lane_line(const SLZ& point, std::vector<XYZ>& roi_points,
                              const double distance = 20.0) const;

  //  ------------------------------------ Route related methods
  //  ----------------------------------------------

  ErrorCode plan_route_shortest(const XYZ& start_point,
                                const std::vector<XYZ>& way_points,
                                const XYZ& end_point, Route& route) const;

  ErrorCode plan_route(const XYZ& start_point,
                       const std::vector<XYZ>& way_points, const XYZ& end_point,
                       Route& route) const;

  /**
   * @brief plan the route from given start/end/way points based on an
   * abstracted Opendrive Map by AStar algorithm.
   *
   * @param start_anchor
  ...
  ErrorCode plan_route(const Anchor& start_anchor,
                       ...
                       const Anchor& end_anchor, Route& route) const;
  /**
   * @brief return a list of sampling points of a user-specified interval on the
   * given route
   *
   * @param route the route which the sample route segment based
   * @param start_route_s the start RouteS of current Sample Route Segment on
   * the route
   * @param length the proposed length of the sample route segment
   * @param default_lane_change_policy for getting the lane change trace without
   * any user input
   * \@param turning_radius the turning radius given by the vehicle state
   * @param sample_route_segment the result route segment sampling point list to
   * be passed out
   * @return ErrorCode: 0 success; otherwise error occurs;
   */
  ErrorCode sample_route(const Route& route, const RouteS& start_route_s,
                         const RouteS& length, SampledLine& sampled_route,
  ...
  /**
   * @brief calclate whether the given point is inside a specified junction or
   * not
   *
   * @param junction_id
   * @param pos
   * @return true: the point is inside the junction (including the situation
   * point on the boundary), false: not inside the junction.
   */
   ...
   * @param junction_id
   * @param junction_boundary XYZ vertices series in vehicle-heading-wise
   * direction, begin with the nearest junction boundary point
   * @return ErrorCode:0 successfully found the jucntion boundary; otherwise
   * error occurs;
   */
  ErrorCode find_junction_boundary(const JunctionId& junction_id,
                                   XYZArray& junction_boundary) const;

  // ------------------------------------ Object related methods
  // ----------------------------------------------

  /**
   * @brief return a list of objects sorted by the (forward) distance to the
   * vehicle on the located route
   * @param route
   * @param start_route_s
   * @param length
   ...
   * @return ErrorCode: 0 success; otherwise error occurs;
   */
  ErrorCode find_objects_on_route(const Route& route,
                                  const RouteS& start_route_s,
                                  const RouteS& length,
                                  const ObjectTypeArray& object_type_filter,
                                  ObjectInRouteArray& object_list) const;

  ErrorCode find_objects_on_road(const RoadId& road,
                                 std::vector<ObjectOnRoad>& objects) const;
  ...
  ErrorCode query_road_mark_types_between(const SLZ& pos1, const SLZ& pos2,
                                          char**& road_mark_types,
                                          unsigned int& length_) const;

  ErrorCode find_slz_with_hint(const XYZ& xyz, const LaneId& hint,
                               SLZ& slz) const;
  ErrorCode calc_slz_with_road_id(const XYZ& xyz, const RoadId& road_id,
                                  SLZ& slz) const;
  ErrorCode find_slz_global(const XYZ& xyz, SLZ& slz) const;

  ErrorCode find_slzs(const XYZ& xyz, std::vector<SLZ>& slzs) const;

  ErrorCode is_road_in_junction(const RoadId& road_id,
                                JunctionId& junction_id) const;
  ...
  double calc_dist_to_end_anchor(const XYZ& xyz,
                                 const std::vector<LaneId>& lane_uids,
  ...
  //
  ErrorCode is_inside_junction(const LaneId& hint, const XYZ& pos,
                               JunctionId& junction_id) const;

  //
  ErrorCode query_lane_linkage(const LaneId& lane_uid,
                               LaneLinkage& lane_linkage) const;

  /*ErrorCode query_lane_neighbor(const LaneId& lane_uid, LaneNeighbor&
   * lane_neighbor) const;*/
  ...
  /**
                   * @brief
     给定阈值范围内,判断平面上某个点是否处于指定道路的frenet坐标系内
                   * @param road_id 指定道路的id
                   * @param xyz 查询点的绝对坐标
                   * @param threshold
     指定点到边界的阈值，在阈值内,表示在道路附近,local_id赋值为离它最近车道的local_id
                   * @param slz 查询点在指定road的frenet坐标系下的相对坐标。
                                            当该点未投影到road参考线上时，返回的slz的lane_id的section_idx和local_id均为INT_MAX；
                                            当该点投影到road参考线上，但是在道路边界外,并在阈值之外时:
     返回的slz的lane_id的local_id为999，section_idx正常；
                                            当该点投影到road参考线上，且在道路边界内时,或者在道路边界外并在阈值之内时:
     返回的slz正常；
                   * @pre-condition 地图已成功加载，road_id
     存在于地图上，distance>0
                   * @post-condition 无
                   * @return 若点能够投影到道路参考线上则返回true，否则返回false
                  */
  bool is_point_in_road_range(const RoadId& road_id, const XYZ& xyz,
                              double threshold, SLZ& slz) const;

  ErrorCode query_lane_boundaries(const LaneId& lane_id, const RoadS& s1,
                                  const RoadS& s2, double sampling_spacing,
                                  SLZArray& left_boundary,
                                  SLZArray& right_boundary) const;

  //
  ErrorCode calc_lane_center_line_curv(
      const LaneId& id, const RoadS& s1, const RoadS& s2,
      double sampling_spacing, std::vector<TracePoint>& centerline) const;

  void get_all_street_lamps(std::vector<StreetLamp>& street_lamps) const;
  void get_street_lamps_by_road(const RoadId road_id,
                                std::vector<StreetLamp>& street_lamps) const;
  ...
  bool get_bus_stop_by_lane(const LaneId& id,
                            std::vector<BusStop>& bus_stops) const;

  bool get_bus_stop_by_road(const RoadId road_id,
                            std::vector<BusStop>& bus_stops) const;

  ErrorCode get_all_parking_space(
      std::vector<ParkingSpace>& parking_spaces) const;
  bool get_parking_space_by_road(
      const RoadId road_id, std::vector<ParkingSpace>& parking_spaces) const;
  bool is_point_in_parking_space(const XYZ& xyz) const;
  bool is_point_in_parking_space(const XYZ& xyz,
                                 const ParkingSpace& parking_space) const;
  bool get_parking_space_by_xyz(const XYZ& xyz,
                                ParkingSpace& parking_space) const;
  ...
  ErrorCode get_rotation_by_xyz_with_hint(const LaneId& hint, const XYZ& point,
   ...
   * @return ErrorCode: 0 success; otherwise error occurs;
   */
  ErrorCode query_traffic_lights_in_road(
      const RoadId& road_id, std::vector<ObjectId>& traffic_lights_ids) const;
   ...
   * @return ErrorCode: 0 success; otherwise error occurs;
   */
  ErrorCode query_all_traffic_light_groups(
      std::vector<TrafficLightGroup>& traffic_light_groups_id) const;

  /**
   * @brief 输入一个SLZ位置，给出该位置所处车道的转向类型。
   * @param slz
   * @param turn_type
   * 传出转向类型，如果当前车道后继不是junction，则返回LaneTurnType::k;目前不考虑kUnkonwnTurnType，只有ErrorCode
   * 非 kOK 时才有 turn_type = kUnkonwnTurnType。
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode query_lane_turn_type(const SLZ& slz,
                                 LaneTurnTypeOrigin& turn_type) const;
  ...
  /**
          * @brief 输入一个SLZ位置，给出该位置前方第一个junction的Id。
                          1.
     查到junction。此时若距离大于distance则认为前方无junction，返回空JunctionId
     = -1；否则返回对应junctionId
                          2.
     后继未查到junction，认为前方无junction，返回空JunctionId = -1；
          * @param slz
          * @param distance 前方查询距离
          * @param junc_id 传出的junction id
          * @return ErrorCode: 0 success; otherwise error occurs;
          */
  ErrorCode query_forward_junction_by_slz(const SLZ& slz, double distance,
                                          JunctionId& junc_id) const;
   ...
   * @param traffic_light_group
   * 传出的红绿灯组，若前方distance距离内无红绿灯组，返回EmptyTrafficLightGroup
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode query_forward_traffic_light_group_by_slz(
      const SLZ& slz, double distance, TrafficLightType type,
      TrafficLightGroup& traffic_light_group) const;

  /**
   * @brief
   * 输入一个SLZ位置，给出该位置前方第一个停止线的Id。若无,返回EmptyObjectId（空字符串）
   * @param slz
   * @param distance 前方查询距离
   * @param stop_line_id 传出的stopline id
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode query_forward_stopline_by_slz(const SLZ& slz, double distance,
                                          ObjectId& stop_line_id) const;

  /**
   * @brief 通过id查询红绿灯对象
   * @param traffic_light_id
   * @param traffic_light
   * 传出的红绿灯对象，若无该id对应的红绿灯，返回EmptyTrafficLight{id = "-1"}
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode get_traffic_light_info_by_id(const TrafficLightId& traffic_light_id,
                                         TrafficLight& traffic_light) const;

  /**
   * @brief 通过id查询停止线对象
   * @param stopline_id
   * @param stopline
   * 传出的停止线对象，若无该id对应的停止线，返回EmptyStopLine{id = "-1"}
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode get_stopline_info_by_id(const ObjectId& stopline_id,
                                    StopLine& stopline) const;
   ...
   */
  ErrorCode get_traffic_light_group_info_by_id(
      const TrafficLightGroupId& traffic_light_group_id,
      TrafficLightGroup& tlg) const;
   ...
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode query_traffic_light_group_id_by_member(
      const TrafficLightId& traffic_light_id,
      TrafficLightGroupId& traffic_light_group_id) const;
   ...
   * @param turn_type
   * 传出转向类型，只有当目标车道是源车道的后继车道时，才返回LaneTurnType::k，否则以ErrorCode非0表示kUnkonwnTurnType。
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   *//**
		* @brief 输入两个车道，源车道与目标车道，给出从源车道到目标车道的转向类型。
		* @param origin_lane_id 源（起始）车道    
		...
		* @return ErrorCode: 0 nothing wrong; otherwise error occurs;
		*/
  ErrorCode query_lane2lane_turn_type(const LaneId& origin_lane_id,
                                      const LaneId& target_lane_id,
                                      LaneTurnTypeOrigin& turn_type) const;
   ...
   * @return ErrorCode: 0 nothing wrong; otherwise error occurs;
   */
  ErrorCode query_stopline_by_traffic_light_group_id(
      const TrafficLightGroupId& traffic_light_group_id,
      ObjectId& stopline_id) const;
  ...
  ErrorCode query_all_roads_in_junction(const JunctionId& junction_id,
                                        std::vector<RoadId>& road_id_vec) const;

  // ErrorCode query_parking_info(const RoadId& road_id, ParkingSpace&
  // parking_space)  const;
   ...
   */
  ErrorCode query_lane_road_mark(
      const LaneId& lane_id, double s,
      std::pair<std::string, std::string>& road_mark_pair) const;

  ErrorCode calc_spiral_points(const TracePoint& start_point,
                               const TracePoint& end_point, double length,
                               double sampling_spacing,
                               std::vector<TracePoint>& result) const;

  bool is_point_around_road(const RoadId& road_id, const XYZ& xyz,
                            double& distance, SLZ& slz) const;
